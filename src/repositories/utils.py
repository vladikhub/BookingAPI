...
def rooms_ids_for_booking(
    ...
    left_rooms = (
        select(
            ...
            (RoomsModel.quantity - func.coalesce(booked_rooms.c.count_booked, 0)).label(
                "rooms_left"
            ),
        )
    ...
    print(
        "---------",
        left_rooms.compile(compile_kwargs={"literal_binds": True}),
        "--------",
    )
    print(f"{left_rooms=}")
    rooms_ids_for_hotel = select(RoomsModel.id)
    ...
    rooms_ids_to_get = (
        ...
        .filter(left_rooms.c.rooms_left > 0, left_rooms.c.room_id.in_(rooms_ids_for_hotel))
    )
